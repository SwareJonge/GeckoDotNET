...
namespace D2XXDirect
{
    ...
    public class D2XXWrapper
    {
        ...
        [DllImport(FT_DLL_Name)]
        private static extern uint FT_ResetPort(uint ftHandle);

        [DllImport(FT_DLL_Name)]
        private static extern uint FT_CyclePort(uint ftHandle);

        [DllImport(FT_DLL_Name)]
        private static extern uint FT_GetDriverVersion(uint ftHandle, ref uint dwDriverVersion);

        [DllImport(FT_DLL_Name)]
        private static extern uint FT_GetQueueStatus(uint ftHandle, ref uint dwQueueCount);
        ...
        [DllImport(FT_DLL_Name)]
        private static extern uint FT_SetChars(uint ftHandle, byte uEventCh, byte uEventChEn, byte uErrorCh, byte uErrorChEn);
        ...
        {
            uint result = D2XXWrapper.FT_ResetPort(this.handle);
            return ConvertResult(result);
        }

        public FT_STATUS CyclePort()
        {
            uint result = D2XXWrapper.FT_CyclePort(this.handle);
            return ConvertResult(result);
        }

        public FT_STATUS GetQueueStatus(ref uint dwQueueCount)
        {
            uint result = D2XXWrapper.FT_GetQueueStatus(this.handle, ref dwQueueCount);
            return ConvertResult(result);
        }

        public FT_STATUS GetDriverVersion(ref uint dwDriverVersion)
        {
            uint result = D2XXWrapper.FT_GetDriverVersion(this.handle, ref dwDriverVersion);
            return ConvertResult(result);
        }

        public FT_STATUS SetEventChar(byte EventMarker, byte enabled)
        {
            uint result = D2XXWrapper.FT_SetChars(this.handle, EventMarker, enabled, 0, 0);
            return ConvertResult(result);
        }
